...
async function fetchGoogleTrends(keyword: string, serpApiKey: string): Promise<GoogleTrendsResult> {
  const url = `https://serpapi.com/search.json?engine=google_trends&q=${encodeURIComponent(keyword)}&data_type=TIMESERIES&date=${encodeURIComponent('today 12-m')}&api_key=${serpApiKey}`;
